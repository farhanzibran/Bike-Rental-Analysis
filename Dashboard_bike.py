...
data['time_of_day'] = data['hr'].apply(classify_time_of_day)
...
data['rental_group'] = data['cnt'].apply(classify_rental_count)

# **3. Tampilkan hasil clustering**
st.title("Clustering Penyewaan Sepeda")

# Opsi filter
group_filter = st.selectbox("Pilih Kelompok untuk Ditampilkan:", ['Semua', 'Pagi', 'Siang', 'Sore', 'Malam'])
rental_filter = st.selectbox("Pilih Grup Penyewaan:", ['Semua', 'Rendah', 'Sedang', 'Tinggi'])

# Filter data berdasarkan pilihan pengguna
filtered_data = data
if group_filter != 'Semua':
    filtered_data = filtered_data[filtered_data['time_of_day'] == group_filter]

if rental_filter != 'Semua':
    filtered_data = filtered_data[filtered_data['rental_group'] == rental_filter]

st.subheader("Data yang Difilter")
st.dataframe(filtered_data)

# **4. Statistik per grup**
st.subheader("Statistik Kelompok")
group_stats = filtered_data.groupby(['time_of_day', 'rental_group']).agg({
    'cnt': ['mean', 'sum'],
    'hr': 'count'
}).reset_index()
...
st.subheader("Visualisasi Penyewaan Berdasarkan Waktu dan Grup Penyewaan")
st.bar_chart(group_stats.pivot(index='time_of_day', columns='rental_group', values=('cnt', 'mean')))
